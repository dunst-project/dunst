...
{
    switch (field) {
    case S_FULLSCREEN: status.fullscreen = value; break;
    case S_IDLE:       status.idle = value; break;
    default:
        LOG_E("Invalid %s enum value in %s:%d for bool type",
              "dunst_status",
              __FILE__,
              __LINE__);
        break;
...
{
    switch (field) {
    case S_PAUSE_LEVEL: status.pause_level = value; break;
    default:
        LOG_E("Invalid %s enum value in %s:%d for int type",
              "dunst_status",
              __FILE__,
              __LINE__);
        break;
...
{
    switch (reason) {
    case DUNST_TIMER:  return "DUNST_TIMER";
    case DUNST_WAKEUP: return "DUNST_WAKEUP";
...
void reload(char **const configs)
{
    ...
    LOG_M("Reloading settings (with the %s files)",
          length != 0 ? "new" : "old");
...
int dunst_main(int argc, char *argv[])
{
    ...
    char *verbosity =
        cmdline_get_string("-verbosity", NULL, "Minimum level for message");
    log_set_level_from_string(verbosity);
    g_free(verbosity);

    cmdline_usage_append(
        "-conf/-config", "string", "Path to configuration file");
    ...
    print_notifications = cmdline_get_bool(
        "-print/--print", false, "Print notifications to stdout");

    bool startup_notification =
        cmdline_get_bool("-startup_notification/--startup_notification",
                         false,
                         "Display a notification on startup.");

    /* Help should always be the last to set up as calls to cmdline_get_* (as a
     * side effect) add entries to the usage list. */
