...
string_replace_all(const char *needle, const char *replacement, char *haystack)
{
    ...
    while (start) {
        ...
        haystack =
            string_replace_at(haystack, needle_pos, needle_len, replacement);
...
char *string_to_path(char *string)
{
    ...
    case 0: break;
    case WRDE_BADCHAR:
        LOG_W("Expansion of \"%s\" failed. It contains invalid characters.",
              string);
        return string;
    case WRDE_BADVAL:
        LOG_W("Expansion of \"%s\" failed. It contains an undefined variable.",
              string);
        return string;
    case WRDE_CMDSUB:
        LOG_W("Expansion of \"%s\" failed. The requested command substitution "
    ...
    case WRDE_NOSPACE:
        ...
        return string;
    case WRDE_SYNTAX:
        LOG_W("Expansion of \"%s\" failed. It contains invalid syntax.",
              string);
        return string;
    }
    g_free(string);
...
static const char *deprecated_sections_message[] = {
    "The settings from the frame section have been moved to the global "
    "section.", // frame
    "Settings in the shortcuts sections have been moved to the global "
    "section.\nAlternatively you can bind shortcuts in your window manager to "
    "dunstctl commands. For that, see the manual for dunstctl.", // shortcuts
};
