...
const char *enum_to_string_fullscreen(enum behavior_fullscreen in)
{
    ...
    default:
        LOG_E(
            "Invalid %s enum value in %s:%d", "fullscreen", __FILE__, __LINE__);
        break;
    }
}
    ...
    for (int i = 0; i < n->script_count; i++) {
        ...
        if (pid1) {
            ...
            if (pid2) {
                ...
                gchar *n_timeout_str =
                    g_strdup_printf("%" G_GINT64_FORMAT, n->timeout / 1000);
                gchar *n_timestamp_str =
                    g_strdup_printf("%" G_GINT64_FORMAT, n->timestamp / 1000);
                ...
                LOG_W("Unable to run script %s: %s",
                      n->scripts[i],
                      strerror(errno));
                exit(EXIT_FAILURE);
            }
...
int notification_cmp(const struct notification *a, const struct notification *b)
{
    ...
    if (settings.sort == SORT_TYPE_UPDATE
        && settings.origin & ZWLR_LAYER_SURFACE_V1_ANCHOR_BOTTOM) {
        a_order = b;
        b_order = a;
    } else {
        a_order = a;
...
{
    return STR_EQ(a->appname, b->appname) && STR_EQ(a->summary, b->summary)
                                    ...
                                    const char *new_icon)
{
    ...
    if (n->icon_path) {
        GdkPixbuf *pixbuf = get_pixbuf_from_file(
            n->icon_path, n->min_icon_size, n->max_icon_size, draw_get_scale());
        if (pixbuf) {
...
void notification_icon_replace_data(struct notification *n, GVariant *new_icon)
{
    ...
    GdkPixbuf *icon = icon_get_for_data(new_icon,
                                        &n->icon_id,
                                        draw_get_scale(),
                                        n->min_icon_size,
                                        n->max_icon_size);
...
void notification_init(struct notification *n)
{
    ...
    switch (n->urgency) {
    case URG_LOW:  defcolors = settings.colors_low; break;
    case URG_NORM: defcolors = settings.colors_norm; break;
    case URG_CRIT: defcolors = settings.colors_crit; break;
    default:       g_error("Unhandled urgency type: %d", n->urgency);
    }
    if (!COLOR_VALID(n->colors.fg))
        n->colors.fg = defcolors.fg;
    if (!COLOR_VALID(n->colors.bg))
        n->colors.bg = defcolors.bg;
    if (!COLOR_VALID(n->colors.frame))
        n->colors.frame = defcolors.frame;
    ...
    if (g_str_has_prefix(n->summary, "DUNST_COMMAND_")) {
        const char *msg = "DUNST_COMMAND_* has been removed, please switch to "
                          "dunstctl. See #830 for more details. "
...
static void notification_format_message(struct notification *n)
{
    ...
    for (char *substr = strchr(n->msg, '%'); substr && *substr;
        ...
        case 'a':
            notification_replace_single_field(
                &n->msg, &substr, n->appname, MARKUP_NO);
            break;
        case 's':
            notification_replace_single_field(
                &n->msg, &substr, n->summary, MARKUP_NO);
            break;
        case 'b':
            notification_replace_single_field(
                &n->msg, &substr, n->body, n->markup);
            break;
        case 'I':
            icon_tmp = g_strdup(n->iconname);
            notification_replace_single_field(&n->msg,
                                              &substr,
        ...
        case 'i':
            notification_replace_single_field(
                &n->msg, &substr, n->iconname ? n->iconname : "", MARKUP_NO);
            break;
        case 'p':
            if (n->progress != -1)
            ...
            notification_replace_single_field(
                &n->msg, &substr, n->progress != -1 ? pg : "", MARKUP_NO);
            break;
        case 'n':
            if (n->progress != -1)
            ...
            break;
        case '%':
            notification_replace_single_field(&n->msg, &substr, "%", MARKUP_NO);
            break;
        case '\0':
...
void notification_update_text_to_render(struct notification *n)
{
    ...
    if ((n->dup_count > 0 && !settings.hide_duplicate_count)
        && (g_hash_table_size(n->actions) || n->urls)
        ...
        buf = g_strdup_printf("(%d%s%s) %s",
                              ...
                              n->urls ? "U" : "",
                              msg);
    } else if ((g_hash_table_size(n->actions) || n->urls)
               && settings.show_indicators) {
    ...
    gint64 t_delta =
        time_monotonic_now() - (n->timestamp - n->timestamp % S2US(1));
    ...
    if (settings.show_age_threshold >= 0
        ...
        if (hours > 0) {
            new_buf =
                g_strdup_printf("%s (%" G_GINT64_FORMAT "h %" G_GINT64_FORMAT
                                "m %" G_GINT64_FORMAT "s old)",
                                buf,
                                hours,
                                minutes,
                                seconds);
        } else if (minutes > 0) {
            new_buf = g_strdup_printf("%s (%" G_GINT64_FORMAT
                                      "m %" G_GINT64_FORMAT "s old)",
                                      buf,
                                      minutes,
                                      seconds);
            ...
            new_buf =
                g_strdup_printf("%s (%" G_GINT64_FORMAT "s old)", buf, seconds);
        }
...
void notification_do_action(struct notification *n)
{
    ...
    if (g_hash_table_size(n->actions)) {
        ...
        if (strcmp(n->default_action_name, "default") == 0
            && g_hash_table_size(n->actions) == 1) {
