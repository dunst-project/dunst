...
static const char *stack_tag_hints[] = {"synchronous",
                                        "private-synchronous",
...
#define DBUS_METHOD(name)                                                      \
    static void dbus_cb_##name(GDBusConnection *connection,                    \
                               const gchar *sender,                            \
                               GVariant *parameters,                           \
                               GDBusMethodInvocation *invocation)
...
static void
dbus_cb_dunst_NotificationClearHistory(GDBusConnection *connection,
                                       const gchar *sender,
                                       GVariant *parameters,
                                       GDBusMethodInvocation *invocation)
{
                                      ...
                                      GDBusMethodInvocation *invocation)
{
    ...
    for (int i = queues_length_history(); i > 0; i--) {
        ...
        g_variant_builder_add(
            &n_builder, "{sv}", "body", g_variant_new_string(body));
        g_variant_builder_add(
            &n_builder, "{sv}", "message", g_variant_new_string(msg));
        g_variant_builder_add(
            &n_builder, "{sv}", "summary", g_variant_new_string(summary));
        g_variant_builder_add(
            &n_builder, "{sv}", "appname", g_variant_new_string(appname));
        g_variant_builder_add(
            &n_builder, "{sv}", "category", g_variant_new_string(category));
        g_variant_builder_add(&n_builder,
                              "{sv}",
                              "default_action_name",
                              g_variant_new_string(default_action_name));
        g_variant_builder_add(
            &n_builder, "{sv}", "icon_path", g_variant_new_string(icon_path));
        g_variant_builder_add(
            &n_builder, "{sv}", "id", g_variant_new_int32(n->id));
        g_variant_builder_add(
            &n_builder, "{sv}", "timestamp", g_variant_new_int64(n->timestamp));
        g_variant_builder_add(
            &n_builder, "{sv}", "timeout", g_variant_new_int64(n->timeout));
        g_variant_builder_add(
            &n_builder, "{sv}", "progress", g_variant_new_int32(n->progress));
        g_variant_builder_add(
            &n_builder, "{sv}", "urgency", g_variant_new_string(urgency));
        g_variant_builder_add(
            &n_builder, "{sv}", "stack_tag", g_variant_new_string(stack_tag));
        g_variant_builder_add(
            &n_builder, "{sv}", "urls", g_variant_new_string(urls));
...
static void
dbus_cb_dunst_NotificationRemoveFromHistory(GDBusConnection *connection,
                                            const gchar *sender,
                                            GVariant *parameters,
                                            GDBusMethodInvocation *invocation)
{
...
static void
color_entry(const struct color c, GVariantDict *dict, const char *field_name)
{
    char buf[10];
    ...
    for (GSList *iter = rules; iter; iter = iter->next) {
        ...
        if (r->match_dbus_timeout > -1)
            g_variant_dict_insert(
                &dict, "match_dbus_timeout", "i", r->match_dbus_timeout);
        ...
        if (r->desktop_entry)
            g_variant_dict_insert(
                &dict, "desktop_entry", "s", r->desktop_entry);
        if (r->summary)
        ...
        if (r->override_dbus_timeout != -1)
            g_variant_dict_insert(
                &dict, "override_dbus_timeout", "x", r->override_dbus_timeout);
        ...
        if (r->history_ignore != -1)
            g_variant_dict_insert(
                &dict, "history_ignore", "b", BOOL2G(r->history_ignore));
        if (r->set_transient != -1)
            g_variant_dict_insert(
                &dict, "set_transient", "b", BOOL2G(r->set_transient));
        if (r->skip_display != -1)
            g_variant_dict_insert(
                &dict, "skip_display", "b", BOOL2G(r->skip_display));
        if (r->word_wrap != -1)
            g_variant_dict_insert(
                &dict, "word_wrap", "b", BOOL2G(r->word_wrap));
        if (r->ellipsize != -1)
            g_variant_dict_insert(
                &dict,
                "ellipsize",
        ...
        if (r->hide_text != -1)
            g_variant_dict_insert(
                &dict, "hide_text", "b", BOOL2G(r->hide_text));
        ...
        if (r->min_icon_size != -1)
            g_variant_dict_insert(
                &dict, "min_icon_size", "i", r->min_icon_size);
        if (r->max_icon_size != -1)
            g_variant_dict_insert(
                &dict, "max_icon_size", "i", r->max_icon_size);
        if (r->action_name)
        ...
        if (r->set_stack_tag)
            g_variant_dict_insert(
                &dict, "set_stack_tag", "s", r->set_stack_tag);
        if (r->override_pause_level != -1)
            g_variant_dict_insert(
                &dict, "override_pause_level", "i", r->override_pause_level);
                                                         ...
                                                         GVariant *parameters)
{
    ...
    while (actions[num]) {
        if (actions[num + 1]) {
            g_hash_table_insert(
                n->actions, g_strdup(actions[num]), g_strdup(actions[num + 1]));
            num += 2;
        } else {
            LOG_W("Odd length in actions array. Ignoring element: %s",
                  actions[num]);
            break;
        }
    }
    ...
    if ((dict_value = g_variant_lookup_value(
             hints, "desktop-entry", G_VARIANT_TYPE_STRING))) {
    ...
    if ((dict_value =
        ...
        g_variant_unref(dict_value);
    } else if ((dict_value = g_variant_lookup_value(
                    hints, "value", G_VARIANT_TYPE_UINT32))) {
    ...
    for (size_t i = 0; i < sizeof(stack_tag_hints) / sizeof(*stack_tag_hints);
         ++i) {
        if ((dict_value = g_variant_lookup_value(
                 hints, stack_tag_hints[i], G_VARIANT_TYPE_STRING))) {
     ...
     */
    if ((dict_value = g_variant_lookup_value(
             hints, "transient", G_VARIANT_TYPE_BOOLEAN))) {
        n->transient = g_variant_get_boolean(dict_value);
        g_variant_unref(dict_value);
    } else if ((dict_value = g_variant_lookup_value(
                    hints, "transient", G_VARIANT_TYPE_UINT32))) {
        n->transient = g_variant_get_uint32(dict_value) > 0;
        g_variant_unref(dict_value);
    } else if ((dict_value = g_variant_lookup_value(
                    hints, "transient", G_VARIANT_TYPE_INT32))) {
        n->transient = g_variant_get_int32(dict_value) > 0;
        g_variant_unref(dict_value);
    }

    dict_value =
        g_variant_lookup_value(hints, "image-path", G_VARIANT_TYPE_STRING);
    if (!dict_value)
        dict_value =
            g_variant_lookup_value(hints, "image_path", G_VARIANT_TYPE_STRING);
    ...
    dict_value = g_variant_lookup_value(
        hints, "image-data", G_VARIANT_TYPE("(iiibiiay)"));
    if (!dict_value)
        dict_value = g_variant_lookup_value(
            hints, "image_data", G_VARIANT_TYPE("(iiibiiay)"));
    if (!dict_value)
        dict_value = g_variant_lookup_value(
            hints, "icon_data", G_VARIANT_TYPE("(iiibiiay)"));
    if (dict_value) {
    ...
    // Modify these values after the notification is initialized and all rules
    // are applied.
    if ((dict_value =
    ...
    if ((dict_value = g_variant_lookup_value(
             hints, "hlcolor", G_VARIANT_TYPE_STRING_ARRAY))) {
        ...
        g_variant_unref(dict_value);
                    ...
                    hints, "hlcolor", G_VARIANT_TYPE_STRING))) {
        struct color c;
        if (string_parse_color(g_variant_get_string(dict_value, NULL), &c)) {
...
void signal_length_propertieschanged(void)
{
    ...
    if (last_history != history) {
        g_variant_builder_add(
            builder, "{sv}", "historyLength", g_variant_new_uint32(history));
        last_history = history;
        properties_changed = true;
    }
    if (last_waiting != waiting) {
        g_variant_builder_add(
            builder, "{sv}", "waitingLength", g_variant_new_uint32(waiting));
        last_waiting = waiting;
        properties_changed = true;
    }

    if (properties_changed) {
        GVariant *body = g_variant_new(
            "(sa{sv}as)", DUNST_IFAC, builder, invalidated_builder);
                           ...
                           GDBusMethodInvocation *invocation)
{
    ...
    if (!n) {
        ...
        g_dbus_method_invocation_return_dbus_error(
            invocation, FDN_IFAC ".Error", "Cannot decode notification!");
        return;
    }
...
void signal_notification_closed(struct notification *n, enum reason reason)
{
    ...
    if (err) {
        ...
        char *reason_string;
        switch (reason) {
        case REASON_TIME:  reason_string = "time"; break;
        case REASON_USER:  reason_string = "user"; break;
        case REASON_SIG:   reason_string = "signal"; break;
        case REASON_UNDEF: reason_string = "undefined"; break;
        default:           reason_string = "unknown";
        }
                                       ...
                                       gpointer user_data)
{
    ...
    if (STR_EQ(property_name, "paused")) {
        ...
        *error = g_error_new(
            G_DBUS_ERROR, G_DBUS_ERROR_UNKNOWN_PROPERTY, "Unknown property");
        return NULL;
    }
}
                                      ...
                                      gpointer user_data)
{
    ...
    if (targetPauseLevel >= 0) {
        ...
        GVariantBuilder *builder =
            g_variant_builder_new(G_VARIANT_TYPE_VARDICT);
        GVariantBuilder *invalidated_builder =
            g_variant_builder_new(G_VARIANT_TYPE_STRING_ARRAY);
        g_variant_builder_add(builder,
                              "{sv}",
        ...
        g_dbus_connection_emit_signal(
            ...
            "PropertiesChanged",
            g_variant_new(
                "(sa{sv}as)", interface_name, builder, invalidated_builder),
            NULL);
                                 ...
                                 gpointer user_data)
{
    ...
    if (!g_dbus_connection_register_object(connection,
                                           ...
                                           &err)) {
        DIE("Unable to register dbus connection interface '%s': %s",
            introspection_data->interfaces[1]->name,
            err->message);
    }
}
                                     ...
                                     char **vendor)
{
    ...
    if (name || vendor) {
        daemoninfo =
            g_dbus_proxy_call_sync(proxy_fdn,
    ...
    proxy_dbus = g_dbus_proxy_new_sync(connection,
                              ...
                              gpointer user_data)
{
    if (connection) {
        ...
        if (STR_EMPTY(env)) {
            LOG_W("DBUS_SESSION_BUS_ADDRESS is blank. Is the dbus session "
...
void signal_history_removed(guint id)
{
    ...
    if (err) {
        LOG_W("Unable to send NotificationHistoryRemoved signal: %s",
              err->message);
        g_error_free(err);
    }
}
...
void signal_history_cleared(guint n)
{
    ...
    if (err) {
        LOG_W("Unable to send NotificationHistoryCleared signal: %s",
              err->message);
        g_error_free(err);
    }
}
...
void signal_config_reloaded(char **const configs)
{
    ...
    GVariant *body =
        g_variant_new("(^as)", length != 0 ? configs : config_paths);
    GError *err = NULL;

    g_dbus_connection_emit_signal(
        dbus_conn, NULL, FDN_PATH, DUNST_IFAC, "ConfigReloaded", body, &err);
