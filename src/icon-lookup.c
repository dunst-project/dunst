...
int load_icon_theme_from_dir(const char *icon_dir, const char *subdir_theme)
{
    ...
    char *theme_index_dir =
        g_build_filename(icon_dir, subdir_theme, "index.theme", NULL);
    ...
    icon_themes_count++;
    icon_themes =
        g_realloc(icon_themes, icon_themes_count * sizeof(struct icon_theme));
    int index = icon_themes_count - 1;
    icon_themes[index].name =
        g_strdup(section_get_value(ini, &ini->sections[0], "Name"));
    ...
    icon_themes[index].dirs = g_malloc0_n(icon_themes[index].dirs_count,
                                          sizeof(struct icon_theme_dir));
    ...
    for (int i = 0; i < icon_themes[index].dirs_count; i++) {
        ...
        if (icon_themes[index].dirs[i].type == THEME_DIR_SCALABLE) {
            ...
            if (min_size)
                safe_string_to_int(&icon_themes[index].dirs[i].min_size,
                                   min_size);
            else
                icon_themes[index].dirs[i].min_size =
                    icon_themes[index].dirs[i].size;

            const char *max_size = section_get_value(ini, &section, "MaxSize");
            if (max_size)
                safe_string_to_int(&icon_themes[index].dirs[i].max_size,
                                   max_size);
            else
                icon_themes[index].dirs[i].max_size =
                    icon_themes[index].dirs[i].size;

        } else if (icon_themes[index].dirs[i].type == THEME_DIR_THRESHOLD) {
            icon_themes[index].dirs[i].threshold = 2;
            const char *threshold =
                section_get_value(ini, &section, "Threshold");
            if (threshold) {
                safe_string_to_int(&icon_themes[index].dirs[i].threshold,
                                   threshold);
            }
        }
    }
    ...
    if (!STR_EQ(icon_themes[index].name, "Hicolor")) {
        char **inherits =
            string_to_array(get_value(ini, "Icon Theme", "Inherits"), ",");
        ...
        icon_themes[index].inherits_index =
            g_malloc0_n(icon_themes[index].inherits_count, sizeof(int));
        ...
        for (int i = 0; inherits[i] != NULL; i++) {
            ...
            if (icon_themes[index].inherits_index[i] == -1) {
                ...
                // since it may be invalidated after realloc. Use an index
                // instead
                icon_themes[index].inherits_index[i] =
                    load_icon_theme(inherits[i]);
            }
        }
        g_strfreev(inherits);
    }
...
void get_theme_path(void)
{
    ...
    add_paths_from_env(
        theme_path, "XDG_DATA_DIRS", "icons", "/usr/local/share/:/usr/share/");
...
void add_default_theme(int theme_index)
{
    ...
    if (theme_index >= icon_themes_count) {
        LOG_W("Invalid theme index: %i. Theme does not exists.", theme_index);
        return;
    }
    default_themes_count++;
    default_themes_index =
        g_realloc(default_themes_index, default_themes_count * sizeof(int));
...
char *find_icon_in_theme(const char *name, int theme_index, int size)
{
    ...
    for (int i = 0; i < theme->dirs_count; i++) {
        ...
        switch (dir.type) {
        case THEME_DIR_FIXED: match_size = dir.size == size; break;
        ...
        if (match_size) {
            ...
            for (const char **suf = suffixes; *suf; suf++) {
                ...
                char *icon = g_build_filename(theme->location,
                                              theme->subdir_theme,
                                              dir.name,
                                              name_with_extension,
                                              NULL);
...
char *
find_icon_in_theme_with_inherit(const char *name, int theme_index, int size)
{
    ...
    for (int i = 0; i < icon_themes[theme_index].inherits_count; i++) {
        ...
        icon = find_icon_in_theme(
            name, icon_themes[theme_index].inherits_index[i], size);
        if (icon)
            return icon;
    }
    return NULL;
...
char *find_icon_path(const char *name, int size)
{
    ...
    for (int i = 0; i < default_themes_count; i++) {
        char *icon = find_icon_in_theme_with_inherit(
            name, default_themes_index[i], size);
        if (icon)
            return icon;
    }
    return NULL;
