...
{
    struct string_to_enum_def *string_to_enum =
        (struct string_to_enum_def *)data;
    for (int i = 0; string_to_enum[i].string != NULL; i++) {
        if (strcmp(s, string_to_enum[i].string) == 0) {
            *(int *)ret = string_to_enum[i].enum_value;
            LOG_D("Setting enum to %i (%s)",
                  *(int *)ret,
                  string_to_enum[i].string);
            return true;
        }
    }
    return false;
...
int string_parse_list(const void *data, const char *s, void *ret)
{
    ...
    default: LOG_W("Don't know this list type: %i", type); break;
    }
    g_strfreev(arr);
    return success;
}
...
int string_parse_sepcolor(const void *data, const char *s, void *ret)
{
    ...
    if (is_enum) {
        ...
        if (STR_EMPTY(s)) {
            LOG_W("Sep color is empty, make sure to quote the value if it's a "
                  "color.");
            return false;
        }
...
{
    if (STR_EMPTY(s) || *s != '#') {
        LOG_W("A color string should start with '#' and contain at least 3 hex "
              "characters");
        return false;
    }
    ...
    case 4:  bpc = 4; break;
    case 8:  bpc = 8; break;
    default: LOG_W("Invalid color string: '%s'", s); return false;
    }
...
// NOTE: We do minimal checks in this function so the values should be sanitized
// somewhere else
int string_parse_length(void *ret_in, const char *s)
{
    ...
    if (!s_stripped) {
        ...
        if (!success) {
            LOG_W("Specify either a single value or two comma-separated values "
                  "between parentheses");
            return false;
        }
...
bool set_from_string(void *target, struct setting setting, const char *value)
{
    ...
    // target instead
    switch (setting.type) {
    case TYPE_INT:    return safe_string_to_int(target, value);
    case TYPE_DOUBLE: return safe_string_to_double(target, value);
    case TYPE_STRING:
        g_free(*(char **)target);
    ...
    case TYPE_CUSTOM:
        ...
        if (!success)
            LOG_W("Invalid %s value: '%s'", setting.name, value);
        return success;
    case TYPE_PATH:;
        g_free(*(char **)target);
        *(char **)target = string_to_path(g_strdup(value));
        ...
        if (!g_shell_parse_argv(*(char **)target,
                                NULL,
                                (char ***)setting.parser_data,
                                &error)) {
    ...
    case TYPE_LENGTH:
        ...
        if (STR_EQ(setting.name, "offset")
            && string_parse_list(GINT_TO_POINTER(OFFSET_LIST), value, target)) {
            LOG_M("Using legacy offset syntax NxN, you should switch to the "
                  "new syntax (N, N)");
            return true;
        }
        ...
        if (STR_EQ(setting.name, "height") && string_is_int(value)) {
            LOG_M("Setting 'height' has changed behaviour after dunst 1.12.0, "
                  "see https://dunst-project.org/release/#v1.12.0.");
            LOG_M("Legacy height support may be dropped in the future. If you "
                  "want to hide this message transition to");
            LOG_M("'height = (0, X)' for dynamic height (old behaviour "
                  "equivalent) or to 'height = (X, X)' for a fixed height.");
    ...
    case TYPE_COLOR:    return string_parse_color(value, target);
    case TYPE_GRADIENT: return string_parse_gradient(value, target);
    default:
        LOG_W("Setting type of '%s' is not known (type %i)",
              setting.name,
              setting.type);
        return false;
    }
...
void set_defaults(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        ...
        if (!set_setting(allowed_settings[i],
                         allowed_settings[i].default_value)) {
            LOG_E("Could not set default of setting %s",
...
void save_settings(struct ini *ini)
{
    for (int i = 0; i < ini->section_count; i++) {
        ...
        for (int j = 0; j < curr_section.entry_count; j++) {
            ...
            if (setting_id < 0) {
                if (setting_id == -1) {
                    LOG_W("Setting %s in section %s doesn't exist",
                          curr_entry.key,
                          curr_section.name);
                }
                continue;
                ...
                if (is_rule) {
                    ...
                    if (rule_offset_is_modifying(curr_setting.rule_offset)) {
                        LOG_D("Adding rule '%s = %s' to special section %s",
                              ...
                              curr_section.name);
                        set_rule(
                            curr_setting, curr_entry.value, curr_section.name);
                    } else {
                        LOG_W("Cannot use filtering rules in special section. "
                              "Ignoring %s in section %s.",
                              curr_entry.key,
                              curr_section.name);
                    }
                } else {
...
char *
cmdline_get_string(const char *key, const char *def, const char *description)
{
...
char *
cmdline_get_path(const char *key, const char *def, const char *description)
{
    cmdline_usage_append(key, "string", description);
...
char **
cmdline_get_list(const char *key, const char *def, const char *description)
{
                          ...
                          const char *description)
{
    ...
    if (!usage_str) {
        usage_str = g_strdup_printf("%-50s - %s\n", key_type, description);
        g_free(key_type);
        return;
    }
