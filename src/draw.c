...
struct gradient *gradient_alloc(size_t length)
{
    ...
    struct gradient *grad =
        g_rc_box_alloc(sizeof(struct gradient) + length * sizeof(struct color));
...
void draw_setup(void)
{
    ...
    LOG_D("Loaded closest matching font: '%s'",
          pango_font_description_get_family(pango_fdesc));
                                        ...
                                        struct colored_layout *cl_next)
{
    ...
    case SEP_FRAME:
        return COLOR(cl_next->n->urgency > cl->n->urgency ? cl_next : cl,
                     frame);
    case SEP_CUSTOM:     return settings.sep_color.color;
    case SEP_FOREGROUND: return COLOR(cl, fg);
    case SEP_AUTO:       return calculate_foreground_color(COLOR(cl, bg));
    default:
        LOG_E(
            "Invalid %s enum value in %s:%d", "sep_color", __FILE__, __LINE__);
        break;
    }
}
...
static int get_horizontal_text_icon_padding(struct notification *n)
{
    bool horizontal_icon =
        (n->icon
         && (n->icon_position == ICON_LEFT || n->icon_position == ICON_RIGHT));
...
// @param width The avaiable text width in pixels, used for caluclating
                               ...
                               PangoAlignment alignment)
{
    ...
    pango_layout_set_spacing(layout,
                             round(settings.line_height * scale * PANGO_SCALE));
...
static void
layout_setup(struct colored_layout *cl, int width, int height, double scale)
{
    int horizontal_padding = get_horizontal_text_icon_padding(cl->n);
    int icon_width =
        cl->icon ? get_icon_width(cl->icon, scale) + horizontal_padding : 0;
    int text_width = width - 2 * settings.h_padding
                   - (cl->n->icon_position == ICON_TOP ? 0 : icon_width);
    int progress_bar_height =
        have_progress_bar(cl) ? settings.progress_bar_height + settings.padding
                              : 0;
    int max_text_height = MAX(
        0, settings.height.max - progress_bar_height - 2 * settings.padding);
    layout_setup_pango(cl->l,
                       text_width,
                       max_text_height,
                       cl->n->word_wrap,
                       cl->n->ellipsize,
...
calculate_notification_dimensions(struct colored_layout *cl, double scale)
{
...
static struct dimensions calculate_dimensions(GSList *layouts)
{
    ...
    if (settings.gap_size) {
        ...
        int extra_gap_height =
            (layout_count * settings.gap_size) - settings.gap_size;
...
static struct colored_layout *
layout_derive_xmore(cairo_t *c, struct notification *n, int qlen)
{
                                                       ...
                                                       struct notification *n)
{
    ...
    GError *err = NULL;
    pango_parse_markup(
        n->text_to_render, -1, 0, &(cl->attr), &(cl->text), NULL, &err);
...
static GSList *create_layouts(cairo_t *c)
{
    ...
    for (const GList *iter = queues_get_displayed(); iter; iter = iter->next) {
        ...
        if (!iter->next && xmore_is_needed
            && settings.notification_limit == 1) {
            char *new_ttr =
                g_strdup_printf("%s (%d more)", n->text_to_render, qlen);
            g_free(n->text_to_render);
            n->text_to_render = new_ttr;
        }
        layouts = g_slist_append(layouts, layout_from_notification(c, n));
    }

    if (xmore_is_needed && settings.notification_limit != 1) {
        /* append xmore message as new message */
        layouts = g_slist_append(
            layouts, layout_derive_xmore(c, queues_get_head_waiting(), qlen));
    }
 ...
 */
static inline void draw_rect(
    cairo_t *c, double x, double y, double width, double height, double scale)
{
    cairo_rectangle(c,
                    round(x * scale),
                    round(y * scale),
...
/**
 * Create a path on the given cairo context to draw the background of a
 * notification. The top corners will get rounded by `corner_radius`, if `first`
 * is set. Respectably the same for `last` with the bottom corners.
 *
 * TODO: Pass additional frame width information to fix blurry lines due to
 * fractional scaling X and Y can then be calculated as:  x = round(x*scale) +
 * half_frame_width
 */
                       ...
                       enum corner_pos corners)
{
    ...
    // This seems to be needed for a problem that occurs mostly when using
    // cairo_stroke and not cairo_fill. Since the only case where we have
    // partial angles is the progress bar and there we use fill, maybe this can
    ...
    // The trickiest cases to handle are when the width is less than
    // corner_radius and corner_radius * 2, because we have to split the angle
    // for the arc in the rounded corner
    if (width <= corner_radius) {
        double angle1 = 0, angle2 = 0;

        // If there are two corners on the top/bottom they occupy half of the
        // width
    ...
    } else if (width <= corner_radius * 2
               && (corners & C_LEFT && corners & C_RIGHT)) {
        double angle1 = 0, angle2 = 0;
                                          ...
                                          double scale)
{
    ...
    radius_int =
        frame_internal_radius(corner_radius, settings.frame_width, height);
    ...
    cairo_set_source_rgba(
        c, COLOR(cl, bg.r), COLOR(cl, bg.g), COLOR(cl, bg.b), COLOR(cl, bg.a));
    cairo_fill(c);

    cairo_set_operator(c, CAIRO_OPERATOR_SOURCE);

    if (settings.sep_color.type != SEP_FRAME && settings.separator_height > 0
        && (corners & (C_BOT | _C_LAST)) == 0) {
        struct color sep_color = layout_get_sepcolor(cl, cl_next);
        cairo_set_source_rgba(
            c, sep_color.r, sep_color.g, sep_color.b, sep_color.a);
...
static void render_content(
    cairo_t *c, struct colored_layout *cl, int width, int height, double scale)
{
    ...
    if (have_progress_bar(cl)) {
        h_without_progress_bar -=
            settings.progress_bar_height + settings.padding;
    }
    ...
    int text_x = settings.h_padding, text_y = settings.padding;
    ...
    if (cl->icon && cl->n->icon_position != ICON_OFF) {
        int image_width = get_icon_width(cl->icon, scale),
            ...
            image_y = text_y, v_padding = get_vertical_text_icon_padding(cl->n);
        ...
        cairo_set_source_surface(
            c, cl->icon, round(image_x * scale), round(image_y * scale));
        draw_rounded_rect(c,
                          image_x,
                          image_y,
                          image_width,
    ...
    if (have_progress_bar(cl)) {
        ...
        unsigned int frame_width = settings.progress_bar_frame_width,
                     progress_width = MIN(width - 2 * settings.h_padding,
                                          settings.progress_bar_max_width),
                     progress_height =
                         settings.progress_bar_height - frame_width,
                     frame_y = h_without_progress_bar,
                     progress_width_without_frame =
                         progress_width - 2 * frame_width,
                     progress_width_1 =
                         progress_width_without_frame * progress / 100,
        ...
        case PANGO_ALIGN_LEFT: frame_x = settings.h_padding; break;
        ...
        unsigned int x_bar_1 = frame_x + frame_width, x_bar_2 = x_bar_1 + 0.5;

        double half_frame_width = (double)frame_width / 2.0;

        /* Draw progress bar
         * TODO: Modify draw_rounded_rect to fix blurry lines due to fractional
         * scaling Note: for now the bar background is drawn a little bit
         * smaller than the fill, however this solution is not particularly
         * solid (basically subracting a pixel or two)
         */
                                       ...
                                       enum corner_pos corners)
{
    ...
    cairo_surface_t *content = render_background(srf,
                                                 cl,
                                                 cl_next,
                                                 dim.y,
                                                 dim.w,
                     ...
                     int *ret_y)
{
    ...
    case ORIGIN_BOTTOM_RIGHT:
        *ret_x = scr->x + (scr->w - width)
               - round(settings.offset.x * draw_get_scale());
        break;
    case ORIGIN_TOP_CENTER:
    case ORIGIN_CENTER:
    case ORIGIN_BOTTOM_CENTER:
    default:                   *ret_x = scr->x + (scr->w - width) / 2; break;
    }
    ...
    case ORIGIN_BOTTOM_CENTER:
    case ORIGIN_BOTTOM_RIGHT:
        *ret_y = scr->y + (scr->h - height)
               - round(settings.offset.y * draw_get_scale());
        break;
    case ORIGIN_LEFT_CENTER:
    case ORIGIN_CENTER:
    case ORIGIN_RIGHT_CENTER:
    default:                  *ret_y = scr->y + (scr->h - height) / 2; break;
    }
}
...
void draw(void)
{
    ...
    cairo_surface_t *image_surface = cairo_image_surface_create(
        CAIRO_FORMAT_ARGB32, round(dim.w * scale), round(dim.h * scale));
