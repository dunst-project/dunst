 ...
 *
 * Requires to call on the surface flush before and mark_dirty after the
 * execution.
 */
...
cairo_surface_t *gdk_pixbuf_to_cairo_surface(GdkPixbuf *pixbuf)
{
    ...
    cairo_format_t fmt = gdk_pixbuf_get_has_alpha(pixbuf) ? CAIRO_FORMAT_ARGB32
                                                          : CAIRO_FORMAT_RGB24;
    cairo_surface_t *icon_surface =
        cairo_image_surface_create(fmt, width, height);
 ...
 * @return TRUE if the dimensions were updated, FALSE if they were left
 * unchanged
 */
                                ...
                                double scale)
{
    ...
    pixbuf = gdk_pixbuf_new_from_file_at_scale(
        filename, round(w * scale), round(h * scale), TRUE, &error);
...
char *get_path_from_icon_name(const char *iconname, int size)
{
    ...
    if (g_str_has_prefix(iconname, "file://")) {
        ...
        // NOTE: Paths starting with ~ should have already been handled at this
        // point
    ...
    char *start = settings.icon_path, *end, *current_folder, *maybe_icon_path,
         *path = NULL;

    do {
        end = strchr(start, ':');
        if (!end)
            end = strchr(settings.icon_path, '\0'); /* end = end of string */
        ...
        for (const char **suf = suffixes; *suf; suf++) {
            ...
            maybe_icon_path =
                g_build_filename(current_folder, name_with_extension, NULL);
...
GdkPixbuf *icon_get_for_data(
    GVariant *data, char **id, double dpi_scale, int min_size, int max_size)
{
