...
struct section *get_or_create_section(struct ini *ini, const char *name)
{
    ...
    if (!s) {
        ini->section_count++;
        ini->sections = g_realloc(ini->sections,
                                  sizeof(struct section) * ini->section_count);
...
const char *
section_get_value(struct ini *ini, const struct section *s, const char *key)
{
    ...
    while (getline(&line, &line_len, fp) != -1) {
        ...
        *equal = '\0';
        ...
        if (quote) {
            ...
            if (!value_end) {
                LOG_W("Invalid config file at line %d: Missing '\"'.",
                      line_num);
                continue;
            }
        } else {
        ...
        if (!current_section) {
            LOG_W("Invalid config file at line %d: Key value pair without a "
                  "section.",
                  line_num);
            continue;
        }
