...
TEST test_names_valid(void)
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        gchar *error1 = g_strdup_printf(
            "Setting name is null (setting description is \"%s\")",
            allowed_settings[i].description);
        gchar *error2 = g_strdup_printf(
            "Setting name is empty (setting description is \"%s\")",
            allowed_settings[i].description);
...
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        gchar *error1 = g_strdup_printf("Description of setting %s is null",
                                        allowed_settings[i].name);
        gchar *error2 = g_strdup_printf("Description of setting %s is empty",
                                        allowed_settings[i].name);
...
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        gchar *error1 = g_strdup_printf("Type of setting %s is not valid: %i",
                                        allowed_settings[i].name,
                                        allowed_settings[i].type);
...
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        gchar *error1 = g_strdup_printf("Section of setting %s is null",
                                        allowed_settings[i].name);
        gchar *error2 = g_strdup_printf("Section of setting %s is empty",
                                        allowed_settings[i].name);
...
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        gchar *error1 = g_strdup_printf("Default_value of setting %s is null",
                                        allowed_settings[i].name);
        gchar *error2 = g_strdup_printf("Default_value of setting %s is empty",
                                        allowed_settings[i].name);
...
TEST test_value_non_null(void)
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        gchar *error1 =
            g_strdup_printf("Error in settting %s. A setting must have a "
                            "'value' or a 'rule_offset', or both.",
                            allowed_settings[i].name);
        ASSERTm(error1,
                allowed_settings[i].value || allowed_settings[i].rule_offset);
        g_free(error1);
    }
    PASS();
}
...
TEST test_valid_parser_and_data_per_type(void)
{
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        ...
        case TYPE_INT:      ; // no parser and no parser data needed
            gchar *error1 =
                g_strdup_printf("Parser of setting %s should be NULL. It's not "
                                "needed for this type",
                                curr.name);
            gchar *error2 =
                g_strdup_printf("Parser data of setting %s should be NULL. "
                                "It's not needed for this type",
                                curr.name);
        ...
        case TYPE_CUSTOM:; // both parser data and parser are needed
            gchar *error3 =
                g_strdup_printf("Parser of setting %s should not be NULL. It's "
                                "needed for this type",
                                curr.name);
            gchar *error4 =
                g_strdup_printf("Parser data of setting %s should not be NULL. "
                                "It's needed for this type",
                                curr.name);
        ...
        case TYPE_LIST:; // only parser data is needed
            gchar *error5 =
                g_strdup_printf("Parser of setting %s should be NULL. It's "
                                "needed not for this type",
                                curr.name);
            gchar *error6 =
                g_strdup_printf("Parser data of setting %s should not be NULL. "
                                "It's needed for this type",
            ...
            g_free(error6);
            break;
        case TYPE_PATH:; // only parser data is neede, but when it's a rule none
                         // is needed.
            gchar *error7 =
                g_strdup_printf("Parser of setting %s should be NULL. It's "
                                "needed not for this type",
            ...
            gchar *error8 =
                g_strdup_printf("Parser data of setting %s should not be NULL. "
                                ...
                                curr.name);
            bool is_rule =
                !curr.value; // if it doesn't have a 'value' it's a rule
            ...
            break;
        default:;
            gchar *error20 = g_strdup_printf(
                "You should make a test for type %i", curr.type);
            FAILm(error20);
            break;
        }
    }
