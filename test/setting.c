...
TEST test_dunstrc_markup(void)
{
    ...
    const char *e_format =
        "<b>%s</b>\\n%b"; // escape the \n since it would otherwise result in
...
TEST test_dunstrc_nomarkup(void)
{
    ...
    const char *e_format =
        "<b>%s</b>\\n<i>%b</i>"; // escape the \n since it would otherwise
                                 // result in the newline character
    const struct rule *r = get_rule("global");
    const char *got_format = r->format;
    ASSERT_STR_EQ(e_format, got_format);
    ASSERT(settings.indicate_hidden);
...
TEST test_dunstrc_defaults(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(allowed_settings); i++) {
        ...
        snprintf(message,
                 500,
                 "The default of setting %s does not match. Different defaults "
                 "are set in code and dunstrc",
        ...
        case TYPE_LENGTH:
        case TYPE_COLOR:
        case TYPE_GRADIENT: break; // TODO implement these checks as well
        default:            printf("Type unknown %s:%d\n", __FILE__, __LINE__);
        }
        /* printf("%zu\n", offset); */
    }
