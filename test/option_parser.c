...
TEST test_cmdline_get_path(void)
{
    ...
    ASSERT_STR_EQ("default",
                  (ptr = cmdline_get_path("-nonexistent", "default", "desc")));
    g_free(ptr);
...
{
    char *ptr;
    ASSERT_STR_EQ("A simple string from the cmdline",
                  (ptr = cmdline_get_string("-string", "", "")));
    free(ptr);
    ASSERT_STR_EQ("Single_word_string",
                  (ptr = cmdline_get_string("-str/-s", "", "")));
    free(ptr);
    ASSERT_STR_EQ("Default",
                  (ptr = cmdline_get_string("-nonexistent", "Default", "")));
    free(ptr);
    PASS();
}
...
TEST test_cmdline_get_list(void)
{
    ...
    CHECK_CALL(ARRAY_EQ(
        cmp3,
        (ptr = cmdline_get_list("-nonexistent", "A, default, list", ""))));
    g_strfreev(ptr);
    PASS();
}
...
{
    if (2.3 != atof("2.3")) {
        SKIPm("Skipping test_cmdline_get_double, as it seems we're running "
              "under musl+valgrind!");
    }
...
{
    g_free(cmdline_get_string(
        "-msgstring/-ms", "", "A string to test usage creation"));
...
{
    int val = -123;
    const char *inputs[] = {"0", "1", "-1", "12345678", "-12345678"};
    const int results[] = {0, 1, -1, 12345678, -12345678};
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        sprintf(buf, "Failed in round %zu", i);
...
{
    if (2.3 != atof("2.3")) {
        SKIPm("Skipping test_string_to_double, as it seems we're running under "
              "musl+valgrind!");
    }

    double val = -100.0;
    const char *inputs[] = {"0", "1", "-1", "45.8", "-45.8"};
    const double results[] = {0, 1, -1, 45.8, -45.8};
    struct setting s;
    s.type = TYPE_DOUBLE;
...
TEST test_string_to_boolean(void)
{
    ...
    const char *inputs[] = {"0", "1", "true", "True", "false", "off"};
    const int results[] = {0, 1, 1, 1, 0, 0};
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
        sprintf(
            buf, "Failed in round %zu. %i should be %i", i, val, results[i]);
        ASSERT_EQm(buf, val, results[i]);
    }
    PASS();
}
...
TEST test_string_to_boolean_invalid(void)
{
    ...
    static char buf[50];
    for (size_t i = 0; i < G_N_ELEMENTS(invalid_inputs); i++) {
        sprintf(buf, "Failed in round %zu", i);
...
TEST test_string_to_enum(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(ellipsize_enum_data) - 1; i++) {
        sprintf(buf, "Failed in round %zu", i);
...
TEST test_string_to_enum_invalid(void)
{
    ...
    const char *invalid_inputs[] = {"0", "1", "-1", "something", "else"};
...
TEST test_string_to_list(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        for (size_t j = 0; val[j] != MOUSE_ACTION_END; j++) {
            sprintf(buf,
                    "Failed in round %zu, element %zu. Is %i, should be %i",
                    i,
                    j,
                    val[j],
                    results[i][j]);
...
TEST test_string_to_list_invalid(void)
{
    ...
    sprintf(buf,
            "List should not change from invalid values. Expected length %i, "
            "got %i",
            len,
            get_list_len(val));
...
TEST test_string_to_time(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
        sprintf(buf,
                "Failed in round %zu. %" G_GINT64_FORMAT " should be %i",
                i,
                val,
                results[i]);
        ASSERT_EQm(buf, val, results[i]);
...
TEST test_string_to_path(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
        sprintf(
            buf, "Failed in round %zu. %s should be %s", i, val, results[i]);
        ASSERTm(buf, STR_EQ(val, results[i]));
        for (size_t j = 0; results2[i][j] != NULL; j++) {
...
TEST test_string_to_sepcolor(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        sprintf(buf,
                "Failed in round %zu. Expected %i, got %i",
                i,
                results[i].type,
                val.type);
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
        ...
        sprintf(buf,
                "Failed in round %zu. Expected %s, got %s",
                i,
                color_to_string(results[i].color, buf1),
                color_to_string(val.color, buf2));
        ASSERTm(buf,
                (!COLOR_VALID(val.color) && !COLOR_VALID(results[i].color))
                    || COLOR_SAME(results[i].color, val.color));
    }
...
TEST test_string_to_sepcolor_invalid(void)
{
    ...
    ASSERT_EQm(
        "Sep color shouldn't changed from invalid inputs", 123, (int)val.type);
    ASSERTm("Sep color shouldn't changed from invalid inputs",
            !COLOR_VALID(val.color));
    PASS();
}
...
TEST test_string_to_color(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        sprintf(buf,
                "Failed in round %zu. Expected %s, got %s",
                i,
                color_to_string(results[i], buf1),
                color_to_string(val, buf2));
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
...
TEST test_string_to_gradient(void)
{
    ...
    results[0]->colors[0] = (struct color){
        (double)0x12 / 0xff, (double)0x34 / 0xff, (double)0x56 / 0xff, 1.0};

    results[1]->colors[0] = (struct color){
        (double)0xab / 0xff, (double)0x12 / 0xff, (double)0x3c / 0xff, 1.0};

    results[2]->colors[0] = (struct color){
        (double)0xaa / 0xff, (double)0xbb / 0xff, (double)0xcc / 0xff, 1.0};
    results[2]->colors[1] = (struct color){
        (double)0xee / 0xff, (double)0xbb / 0xff, (double)0xff / 0xff, 1.0};
    results[2]->colors[2] = (struct color){
        (double)0xaa / 0xff, (double)0xaf / 0xff, (double)0xff / 0xff, 1.0};

    results[3]->colors[0] = (struct color){
        (double)0xab / 0xff, (double)0xc1 / 0xff, (double)0x23 / 0xff, 1.0};
    results[3]->colors[1] = (struct color){
        (double)0xac / 0xff, (double)0xaf / 0xff, (double)0x8f / 0xff, 1.0};
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        sprintf(buf, "Failed in round %zu. Expected %s, got %s", i, t1, t2);
...
TEST test_string_to_length(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
        sprintf(buf,
                "Failed in round %zu. Expected min to be %i, got %i",
                i,
                results[i].min,
                val.min);
        ASSERT_EQm(buf, results[i].min, val.min);
        sprintf(buf,
                "Failed in round %zu. Expected max to be %i, got %i",
                i,
                results[i].max,
                val.max);
...
TEST test_string_to_corners_invalid(void)
{
    ...
    ASSERT_EQm(
        "Enum should not change from invalid values", corners, C_ALL + 1);
    PASS();
}
...
TEST test_string_to_maybe_int(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(inputs); i++) {
        ...
        ASSERTm(buf, set_from_string(&val, s, inputs[i]));
        sprintf(buf,
                "Failed in round %zu. Expected val to be %s, got %s",
                i,
                results[i].s,
                val);
        ASSERTm(buf, STR_EQ(val, results[i].s));
        sprintf(buf,
                "Failed in round %zu. Expected intval to be %i, got %i",
                i,
                results[i].i,
                intval);
