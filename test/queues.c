...
TEST test_queue_length(void)
{
    ...
    ASSERT_EQm(
        "Queue waiting has to contain an element", 1, queues_length_waiting());
    ASSERT_EQm("Queue displayed has to contain an element",
               1,
               queues_length_displayed());
    ASSERT_EQm(
        "Queue history has to contain an element", 1, queues_length_history());
...
TEST test_queue_insert_id_invalid(void)
{
    ...
    queues_notification_insert(n);
    QUEUE_LEN_ALL(1, 0, 0);
    ASSERTm("The given ID shouldn't be 0 anymore.", n->id > 0);
...
TEST test_queue_notification_skip_display_redisplayed_by_random_id(void)
{
    ...
    for (size_t i = 0; i < notification_buffer_size; i++) {
        if (is_n_popped[i]) {
            QUEUE_CONTAINSm(
                "A skip display notification should stay in displayed "
                "queue when it got pulled out of history queue",
                DISP,
                n[i]);
        } else {
...
TEST test_datachange_beginning_empty(void)
{
    ...
    ASSERTm(
        "There are no notifications at all, the timeout has to be less than 0.",
...
TEST test_datachange_endless(void)
{
    ...
    ASSERTm("Age threshold is deactivated and the notification is infinite, "
            "there is no wakeup necessary.",
            queues_get_next_datachange(time_monotonic_now()) < 0);
...
TEST test_datachange_endless_agethreshold(void)
{
    ...
    ASSERT_IN_RANGEm(
        "Age threshold is activated and the next wakeup should be less than a "
        "second away",
        AGE_THRESHOLD - S2US(1) / 2,
        queues_get_next_datachange(cur_time + AGE_THRESHOLD - S2US(1)),
        S2US(1) / 2);

    ASSERT_IN_RANGEm("Age threshold is activated and the next wakeup should be "
                     "less than the age threshold",
                     AGE_THRESHOLD / 2,
                     queues_get_next_datachange(cur_time),
                     AGE_THRESHOLD / 2);
...
TEST test_datachange_agethreshold_at_second(void)
{
    ...
    for (gint64 base_time = S2US(5); base_time <= S2US(7);
         base_time += S2US(1)) {
        for (int musec_id = 0; musec_id < NB_MICROSECS; ++musec_id) {
            ...
            ASSERTm("Age threshold is activated, next wakeup should be at next "
                    "turn of second",
...
TEST test_datachange_queues(void)
{
    ...
    queues_notification_insert(n);
    ASSERTm("The inserted notification is inside the waiting queue, so it "
            "should get ignored.",
            queues_get_next_datachange(cur_time) < 0);

    queues_update(STATUS_NORMAL, time_monotonic_now());
    ASSERT_IN_RANGEm(
        "The notification has to get closed in less than its timeout",
        S2US(10) / 2,
        queues_get_next_datachange(cur_time),
        S2US(10) / 2);

    queues_notification_close(n, REASON_UNDEF);
    ASSERTm("The inserted notification is inside the history queue, so it "
            "should get ignored",
            queues_get_next_datachange(cur_time) < 0);
...
TEST test_datachange_ttl(void)
{
    ...
    queues_notification_insert(n);
    queues_update(STATUS_NORMAL, cur_time);
    ASSERT_IN_RANGEm("The timeout of the second notification has to get used "
                     "as sleep time now.",
                     cur_time + n->timeout / 2,
                     queues_get_next_datachange(cur_time),
                     n->timeout / 2);

    cur_time += S2US(10);

    ASSERT_EQm("The notification already timed out. You have to answer with "
               "the current time.",
               cur_time,
               queues_get_next_datachange(cur_time));
...
TEST test_queues_update_pause_level(void)
{
    ...
    ASSERT(
        strcmp(
            ((struct notification *)g_queue_peek_nth(QUEUE(DISP), 0))->summary,
            "n3")
        == 0);
    ASSERT(
        strcmp(
            ((struct notification *)g_queue_peek_nth(QUEUE(WAIT), 0))->summary,
            "n1")
        == 0);
    ASSERT(
        strcmp(
            ((struct notification *)g_queue_peek_nth(QUEUE(WAIT), 1))->summary,
            "n2")
        == 0);
