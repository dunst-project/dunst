...
void dbus_signal_subscribe_actioninvoked(
    struct signal_actioninvoked *actioninvoked)
{
    assert(actioninvoked);

    actioninvoked->conn = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
    actioninvoked->subscription_id =
        g_dbus_connection_signal_subscribe(actioninvoked->conn,
                                           FDN_NAME,
                                           FDN_IFAC,
                                           "ActionInvoked",
                                           FDN_PATH,
...
void dbus_signal_unsubscribe_actioninvoked(
    struct signal_actioninvoked *actioninvoked)
{
    assert(actioninvoked);

    g_dbus_connection_signal_unsubscribe(actioninvoked->conn,
                                         actioninvoked->subscription_id);
                                      ...
                                      gpointer user_data)
{
    ...
    struct signal_propertieschanged *sig =
        (struct signal_propertieschanged *)user_data;
...
void dbus_signal_subscribe_propertieschanged(
    struct signal_propertieschanged *propertieschanged)
{
    assert(propertieschanged);

    propertieschanged->conn = g_bus_get_sync(G_BUS_TYPE_SESSION, NULL, NULL);
    propertieschanged->subscription_id = g_dbus_connection_signal_subscribe(
...
void dbus_signal_unsubscribe_propertieschanged(
    struct signal_propertieschanged *propertieschanged)
{
    assert(propertieschanged);

    g_dbus_connection_signal_unsubscribe(propertieschanged->conn,
...
void dbus_signal_subscribe_closed(struct signal_closed *closed)
{
    ...
    closed->subscription_id =
        g_dbus_connection_signal_subscribe(closed->conn,
                                           FDN_NAME,
                                           FDN_IFAC,
                                           "NotificationClosed",
                                           FDN_PATH,
                                   ...
                                   gpointer user_data)
{
    ...
    struct signal_historyremoved *sig =
        (struct signal_historyremoved *)user_data;
...
void dbus_signal_subscribe_historyremoved(struct signal_historyremoved *sig)
{
    ...
    sig->subscription_id =
        g_dbus_connection_signal_subscribe(sig->conn,
                                           FDN_NAME,
                                           DUNST_IFAC,
                                   ...
                                   gpointer user_data)
{
    ...
    struct signal_historycleared *sig =
        (struct signal_historycleared *)user_data;
                                   ...
                                   gpointer user_data)
{
    ...
    struct signal_configreloaded *sig =
        (struct signal_configreloaded *)user_data;
...
static GVariant *
dbus_invoke_ifac(const char *method, GVariant *params, const char *ifac)
{
    ...
    retdata = g_dbus_connection_call_sync(connection_client,
                                          FDN_NAME,
                                          FDN_PATH,
                                          ifac,
                                          method,
...
struct dbus_notification *dbus_notification_new(void)
{
    ...
    n->hints = g_hash_table_new_full(
        g_str_hash, g_str_equal, g_free, g_free_variant_value);
    return n;
}
...
void dbus_notification_set_raw_image(struct dbus_notification *n_dbus,
                                     const char *path)
{
    ...
    g_hash_table_insert(
        n_dbus->hints, g_strdup("image-data"), g_variant_ref_sink(hint));
}
...
TEST test_dbus_cb_dunst_NotificationListHistory(void)
{
    ...
    GVariant *result =
        dbus_invoke_ifac("NotificationListHistory", NULL, DUNST_IFAC);
    ASSERT(result != NULL);
...
TEST test_dbus_cb_dunst_RuleEnable(void)
{
    ...
    GVariant *result = dbus_invoke_ifac(
        "RuleEnable", g_variant_new("(si)", "test_rule_enable", 0), DUNST_IFAC);
    ASSERT(result != NULL);
    ASSERT(!rule->enabled);
    g_variant_unref(result);

    result = dbus_invoke_ifac(
        "RuleEnable", g_variant_new("(si)", "test_rule_enable", 1), DUNST_IFAC);
    ASSERT(result != NULL);
    ASSERT(rule->enabled);
    g_variant_unref(result);

    result = dbus_invoke_ifac(
        "RuleEnable", g_variant_new("(si)", "test_rule_enable", 2), DUNST_IFAC);
    ASSERT(result != NULL);
    ASSERT(!rule->enabled);
    g_variant_unref(result);
...
TEST test_basic_notification(void)
{
    ...
    n->body = "Text";
    g_hash_table_insert(
        n->actions, g_strdup("actionid"), g_strdup("Print this text"));
...
TEST test_dbus_notify_colors(void)
{
    ...
    struct color frame = {
        1.0, (double)0xaa / 0xff, (double)0xcc / 0xff, (double)0xbb / 0xff};
    ...
    ASSERTm("Valid color strings should change the notification color",
            COLOR_SAME(n->colors.frame, frame));
    ASSERTm("Valid color strings should change the notification color",
            COLOR_SAME(n->colors.fg, fg));

    // Invalid color strings are ignored
    ASSERTm("Invalid color strings should not change the color struct",
            COLOR_SAME(n->colors.bg, settings.colors_norm.bg));

    ASSERTm("Invalid color strings should not change the gradient struct",
            n->colors.highlight->length
                == settings.colors_norm.highlight->length);

    for (size_t i = 0; i < settings.colors_norm.highlight->length; i++)
        ASSERTm("Invalid color strings should not change the gradient struct",
                COLOR_SAME(n->colors.highlight->colors[i],
                           settings.colors_norm.highlight->colors[i]));
...
TEST test_hint_progress(void)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(variants); i++) {
        g_hash_table_insert(
            n_dbus->hints, g_strdup("value"), g_variant_ref_sink(variants[i]));
        ...
        snprintf(msg,
                 sizeof(msg),
                 "In round %zu progress should be %i, but is %i",
                 i,
                 n->progress,
                 values[i]);
...
TEST test_signal_actioninvoked(void)
{
    ...
    n_dbus->body = "Text";
    g_hash_table_insert(
        n_dbus->actions, g_strdup("actionkey"), g_strdup("Print this text"));
...
TEST test_signal_length_propertieschanged(void)
{
    ...
    g_variant_lookup(
        sig.array_dict_sv_data, "displayedLength", "u", &displayed_length);
...
TEST assert_methodlists_sorted(void)
{
    for (size_t i = 0; i + 1 < G_N_ELEMENTS(methods_fdn); i++) {
        ASSERT(0 > strcmp(methods_fdn[i].method_name,
                          methods_fdn[i + 1].method_name));
    }

    for (size_t i = 0; i + 1 < G_N_ELEMENTS(methods_dunst); i++) {
        ASSERT(0 > strcmp(methods_dunst[i].method_name,
