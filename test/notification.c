...
TEST test_notification_replace_single_field(void)
{
    ...
    notification_replace_single_field(
        &str, &substr, "and &amp; <i>is</i>", MARKUP_FULL);
    ...
    substr = strchr(str, '%');
    notification_replace_single_field(
        &str, &substr, "and & <i>is</i>", MARKUP_NO);
    ...
    substr = strchr(str, '%');
    notification_replace_single_field(
        &str, &substr, "<i>is removed</i> and & escaped", MARKUP_STRIP);
...
notification_load_icon_with_scaling(int min_icon_size, int max_icon_size)
{
...
TEST test_notification_format_message(struct notification *n,
                                      const char *format,
                                      const char *exp)
{
...
TEST test_notification_maxlength(void)
{
    ...
    static const char sigma[] = " 0123456789"
