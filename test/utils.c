...
TEST test_string_replace_all(void)
{
    ...
    strcpy(text, "Nothing to replace");
    ASSERT_STR_EQ((text = string_replace_all("z", "a", text)),
                  "Nothing to replace");

    strcpy(text, "Reverse this");
    ASSERT_STR_EQ("Reverse sith",
                  (text = string_replace_all("this", "sith", text)));

    strcpy(text, "abcdabc");
    ASSERT_STR_EQ("xyzabcdxyzabc",
                  (text = string_replace_all("a", "xyza", text)));
...
TEST test_string_append(void)
{
    ...
    ASSERT_STR_EQ("text_sep_bit",
                  (exp = string_append(g_strdup("text"), "bit", "_sep_")));
    g_free(exp);
    ASSERT_STR_EQ("textbit",
                  (exp = string_append(g_strdup("text"), "bit", NULL)));
    g_free(exp);
    ASSERT_STR_EQ("textbit",
                  (exp = string_append(g_strdup("text"), "bit", "")));
    g_free(exp);
...
TEST test_string_strip_quotes(void)
{
    ...
    ASSERT_STR_EQ("becomes unquoted",
                  (exp = string_strip_quotes("\"becomes unquoted\"")));
    g_free(exp);

    ASSERT_STR_EQ("\"stays quoted",
                  (exp = string_strip_quotes("\"stays quoted")));
    g_free(exp);
...
TEST test_string_strip_delimited(void)
{
    ...
    string_strip_delimited(text, '<', '>');
    ASSERT_STR_EQ("Nothing is done if there are no delimiters in the string",
                  text);
...
TEST test_string_to_path(void)
{
    ...
    // This might fail, when a user named path exists on the host running the
    // tests.
    ...
    ASSERT_STR_EQ(
        (exp = g_strconcat(home, "/.path/with/tilde and some space", NULL)),
        (ptr = string_to_path(g_strdup("~/.path/with/tilde and some space"))));
    free(exp);
    free(ptr);

    ASSERT_STR_EQ((exp = g_strconcat(
                       home, "/.path/with/HOME environment variable", NULL)),
                  (ptr = string_to_path(g_strdup(
                       "$HOME/.path/with/HOME environment variable"))));
    free(exp);
    free(ptr);
...
TEST test_string_to_time(void)
{
    char *input[] = {"5000 ms",
                     ...
                     NULL};
    gint64 exp[] = {
        5000, 5000, 100000, 10000, 120000, 39600000, 777600000, 5, 0};
