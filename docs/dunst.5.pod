...
=item B<gaps> (values: [true/false], default: false)

If set to true, display a gap of size B<gap_size> between notifications.

This setting requires a compositor and any applications displayed between the gaps are not currently clickable.

=item B<gap_size> (default: 10)

Size of gap to display between notifications if B<gaps> is set to true.
