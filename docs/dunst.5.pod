...
This setting is deprecated. It's split up into B<width>, B<height>, B<origin>,
...
Please note that the path should now be the root of the icon theme, not a
subdirectory of it. This has changed in version 1.7 and you should change
your icon path accordingly.
The directory will be searched recursively for the most appropriate match based
on icon size and scale. Because of this recursive search adding too many
large icon themes to the path may result in bad performance. Try to stick to one
or two icon themes in the path.
