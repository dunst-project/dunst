...
=item B<default_pause_level> (values: [0-100], default: 0)

Specify a pause level to be set on dunst startup. Notifications are only shown if
their associated pause level is higher than the pause level threshold.
See B<override_pause_level> for more information.
...
If this is set to false (not recommended), dunst will use fnmatch(3) for matching
...
Examples:
    ...
    height = (300, 300) # constant height of 300
    height = (0, 300)   # height between 0 and 300

Note that unlike width, different notifications can have diffrent height values.

Dunst v1.11 and older do not support a dynamic height and B<height> accepts a
single value whose behaviour is equivalent to (0, value). A single value is
interpreted in this way to maintain backwards compatibility.
...
This setting will be ignored if B<progress_bar_corners> is set to none.
...
This setting will be ignored if B<icon_corners> is set to none.
...
Setting this to a non-zero value overwrites any padding that horizontal_padding was
adding between the notification text and icon.
...
=item B<layer> (values: [bottom/top/overlay], default: overlay) (Wayland only)
...
This options is parsed as a Pango font description. More information on the string
format can be found here https://docs.gtk.org/Pango/type_func.FontDescription.from_string.html.
...
C<format = "">. See SCRIPTING.

=item B<title> (default: "Dunst") (X11 only)

Defines the title (I<_NET_WM_NAME> property) of notification windows spawned by dunst.

There should be no need to modify this setting for regular use.

=item B<class> (default: "Dunst") (X11 only)

Defines the class (I<WM_CLASS> first part) of notification windows spawned by dunst.
...
setting is I<provided for compatibility> with older nVidia drivers that do not
support RandR and using it on systems that support RandR is I<highly discouraged>.
...
single notification, unless if gaps are enabled (see B<gaps_size>).
...
This setting will be ignored if B<corners> is set to none.

=item B<corners> (values: [none/all/bottom-right/bottom-left/top-right/top-left/top/bottom/left/right], default: all)
...
Ignore the dbus I<CloseNotification> message. This is useful to enforce the timeout
...
A notification will appear whenever notification's I<override_pause_level> is greater than the paused level.
Setting this to values other than 0 allows you to create partial pause modes, where more urgent notifications get through,
but less urgent stay paused.

For example, when you can set a low battery notification's override_pause_level to 60 and then
set dunst's pause level to 60. This will cause dunst to only show battery level notification
(and other notifications with override_pause_level >= 60), while suspending others.
...
=item B<pause_on_mouse_over> (default: false)
...
It's recommended to use the C<desktop_entry> filter instead.
...
A modifying rule, similarly to a filtering one, is made by assigning a value
to a notification attribute in the rule definition.
...
=item C<fullscreen> (values: [delay/show/pushback/suppress], default: show)

This attribute specifies how notifications are handled if a fullscreen window
is focused. By default it's set to I<show> so notifications are being shown.

Other possible values are I<delay>: already shown notifications are continued to be
...
Or I<pushback> which is equivalent to delay with the difference that already
existing notifications are paused and hidden until the focus to the fullscreen
window is lost.

There is additionally the I<suppress> value. Existing notifications will be closed
when entering fullscreen and new notifications will go straight to history.
This is similar to pushback without redisplaying after leaving fullscreen mode.
...
=item C<min_icon_size> (default: 32)
...
=item C<max_icon_size> (default: 128)
...
=item C<action_name> (default: "default")
...
=item C<word_wrap> (values: [true/false], default: true)
...
=item C<ellipsize> (values: [start/middle/end], default: middle)

Specifies where truncated lines should be ellipsized.

=item C<alignment> (values: [left/center/right], default: left)

Defines how the text should be aligned within the notification.

=item C<hide_text> (values: [true/false], default: false)
...
=item C<markup> (values: [full/strip/no], default: no)
...
Note: Dunstify is compatible with notify-send and all of these
