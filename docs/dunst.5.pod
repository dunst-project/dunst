...
Any other value is interpreted as a color, see B<COLORS>.
...
See B<TIME FORMAT> for valid times.
...
For example, when you can set a low battery noficiation's override_pause_level to 60 and then
...
See B<RULES> for matching specific broken clients.
...
Note: Dunstify is a notify-send alternative compatible with all of these settings.
