...
Transient is boolean value that any notification client can set when deemed
appropriate. Dunst does modify its behaviour if it's set by default but you can
match it in rules if necessary
...
=item C<idle_threshold>

This version of idle_threshold is currently more limited than the global one.
The possible values are -1, which respects the existing global value or 0 which
makes the notification bypass it and timeout anyway.
